...
class InvoiceGenerator:
    ...
    def _generate_invoice_pdf(self, company_name, company_type, invoice_num, date,
        ...
        if layout != "modern":
            ...
            pdf.cell(135, 7, f'TVA ({tva_rate}%):', 0, 0, 'R')
            pdf.cell(55, 7, f'{tva:.2f} EUR', 0, 1, 'R')

            pdf.set_font(font, 'B', 12)
            pdf.cell(135, 10, 'Total TTC:', 0, 0, 'R')
            pdf.cell(55, 10, f'{total_ttc:.2f} EUR', 0, 1, 'R')
        else:
        # Modern layout has a different totals display
            ...
            pdf.cell(135, 8, f'TVA ({tva_rate}%):', 0, 0, 'R')
            pdf.cell(55, 8, f'{tva:.2f} EUR', 0, 1, 'R')

            pdf.set_font(font, 'B', 12)
            pdf.set_fill_color(220, 220, 220)
            pdf.cell(135, 10, 'Total TTC:', 0, 0, 'R', 1)
            pdf.cell(55, 10, f'{total_ttc:.2f} EUR', 0, 1, 'R', 1)
    ...
    def _generate_letter(self, output_path):
        ...
        pdf.cell(0, 5, fake.phone_number(), ln=True)
        
        # Date
        pdf.ln(10)
        date = datetime.now().strftime("%d %B %Y")
        pdf.cell(0, 5, date, ln=True, align='R')
        
        # Recipient
        pdf.ln(10)
        pdf.cell(0, 5, fake.name(), ln=True)
        pdf.cell(0, 5, fake.job_title(), ln=True)
        ...
        pdf.ln(10)
        pdf.set_font(font, 'B', 11)
        pdf.cell(0, 8, f"Objet: {random.choice(['Demande de renseignements', 'Suivi de projet', 'Invitation', 'Confirmation de rendez-vous'])}", ln=True)
        ...
        # Closing
        pdf.ln(10)
        pdf.cell(0, 5, "Cordialement,", ln=True)
        pdf.ln(10)
        pdf.cell(0, 5, fake.name(), ln=True)
        pdf.cell(0, 5, fake.job_title(), ln=True)
    ...
    def _generate_memo(self, output_path):
        ...
        pdf.cell(0, 8, datetime.now().strftime("%d/%m/%Y"), 1, 1, 'L')
        
        pdf.cell(40, 8, "De:", 1, 0, 'L', 1)
        pdf.cell(0, 8, fake.name(), 1, 1, 'L')
        
        pdf.cell(40, 8, "À:", 1, 0, 'L', 1)
        pdf.cell(0, 8, random.choice(["Tous les employés", "Service comptabilité", "Équipe marketing", "Direction"]), 1, 1, 'L')
        
        pdf.cell(40, 8, "Objet:", 1, 0, 'L', 1)
        pdf.cell(0, 8, random.choice(["Réunion mensuelle", "Procédures de sécurité", "Nouveaux horaires", "Événement d'entreprise"]), 1, 1, 'L')
        ...
        # Signature
        pdf.ln(15)
        pdf.cell(0, 6, fake.name(), ln=True)
        pdf.cell(0, 6, fake.job_title(), ln=True)
    ...
    def _generate_report(self, output_path):
        ...
        pdf.cell(0, 8, f"{month} {year}", ln=True, align='C')
    ...
    def _generate_form(self, output_path):
        ...
        pdf.cell(0, 10, form_type, ln=True, align='C')
